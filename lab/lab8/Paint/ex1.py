...
class DrawingScene(SceneBase):
    def __init__(self):
        ...
        self.shapes = []  # List to hold shapes
        self.current_color = (0, 0, 0)  # Default color
        self.current_tool = "Rectangle"  # Default drawing tool
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:  # Left mouse button
                    if self.current_tool == "Rectangle":
                        ...
                        rect = pygame.Rect(pos[0], pos[1], 50, 50)  # Default size
                        self.shapes.append((rect, self.current_color))
                    elif self.current_tool == "Circle":
                        pos = pygame.mouse.get_pos()
                        circle = (pos[0], pos[1], 25)  # Default radius
                        self.shapes.append((circle, self.current_color))
                    elif self.current_tool == "Eraser":
                        # Check if any shape is clicked and remove it
                        pos = pygame.mouse.get_pos()
